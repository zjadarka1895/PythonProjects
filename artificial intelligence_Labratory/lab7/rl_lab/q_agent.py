...
class QAgent(Agent):
    ...
    def __init__(self, n_states, n_actions,
        ...
        self.epsilon = 10**(-2)           # epsilon (p-wo akcji losowej)
        self.eps_decrement = 10**(-6)     # wartość, o którą zmniejsza się epsilon po każdym kroku
        self.eps_min = 10**(-4)           # końcowa wartość epsilon, poniżej którego już nie jest zmniejszane
    ...
    def choose_action(self, state: State) -> Action:
        ...
        if np.random.uniform(0, 1) < self.epsilon:
            action = Action(np.random.choice(self.action_space))
        else:
    ...
    def learn(self, state: State, action: Action, reward: float, new_state: State, done: bool) -> None:
        ...
        max_future_q = np.max(self.q_table[new_state]) if not done else 0.0
        delta = reward + self.gamma * max_future_q - current_q
