#1. jak są trzy moje niezasłonięte to wrzucaj
#2. jak są trzy jego to wrzucać
#3. jak są moje 2 to git
#4. lepiej środek niż z brzegu
    ...
    def minmax(self, board: 'Connect4', x: int, d: int) -> Tuple[int, int]:
        ...
        for possible_move in board.possible_drops():
            ...
            if x == 1:
                ...
                score += self.heuristic(simulation, possible_move)
                if score > best_score:
                    best_score = score
                    best_move = possible_move
            else:
                score, _ = self.minmax(simulation, 1, d - 1)
                score += 3 - abs(possible_move - 3)
                if score < best_score:
                    best_score = score
                    best_move = possible_move
